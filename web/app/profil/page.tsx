...
export default function ProfilePage() {
  const [profile, setProfile] = useState<Profile | null>(null); const [message,setMessage]=useState(''); const [error,setError]=useState('');
  useEffect(()=>{ api<Profile>('/profile').then(setProfile).catch(e=>setError(e.message)); },[]);
  if (!profile) return error ? <ErrorBox message={error}/> : <Loading/>;
  const set = (key:keyof Profile, value:any) => setProfile({...profile,[key]:value});
  const save = async () => { setError(''); try { setProfile(await api<Profile>('/profile',{method:'PUT',body:JSON.stringify(profile)})); setMessage('Profil enregistré.'); } catch(e:any){setError(e.message);} };
  return <>
    <PageHeader title="Profil candidat" description="Ces données servent uniquement à remplir les formulaires et générer les messages." actions={<button className="btn" onClick={save}>Enregistrer</button>} />
    {message && <div className="notice">{message}</div>}{error && <ErrorBox message={error}/>}<div style={{height:14}}/>
    <Card><div className="form-grid">
      ...
      <label className="full">Langues (une ligne par langue : niveau)<textarea value={profile.languages.map(l=>`${l.language}: ${l.level}`).join('\n')} onChange={e=>set('languages',e.target.value.split('\n').filter(Boolean).map(line=>{const [language,...rest]=line.split(':');return {language:language.trim(),level:rest.join(':').trim()}}))}/></label>
    </div></Card>
  </>;
}
