...
- The status is visible and recoverable from the existing Applications status filtering while a dedicated ignored Offers view is kept as a later small UX slice.
...
No database migration or API contract change is required: application status is already persisted as a string and the existing PATCH workflow accepts local tracking states.
