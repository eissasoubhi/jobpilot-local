...
final class CvController
{
    ...
    public function list(): JsonResponse
    {
        ...
        return new JsonResponse(array_map(static fn(CvDocument $cv) => $cv->toArray(), $items));
    }
    ...
    public function upload(Request $request): JsonResponse
    {
        ...
        if ($file === null) throw new \InvalidArgumentException('Le fichier CV est obligatoire.');
        $language = strtolower((string) $request->request->get('language', 'fr'));
        if (!in_array($language, ['fr','en'], true)) throw new \InvalidArgumentException('Langue CV invalide.');
        $originalName = $file->getClientOriginalName();
        $mimeType = $file->getMimeType() ?: 'application/octet-stream';
        $size = $file->getSize() ?: 0;
        $stored = $this->storage->store($file);
        $cv = new CvDocument(
            (string) $request->request->get('name', pathinfo($file->getClientOriginalName(), PATHINFO_FILENAME)),
            $originalName, $stored, $language,
            $mimeType, $size
        );
        ...
        $cv->configure([
            ...
            'tags' => $tags,
            'defaultForLanguage' => filter_var($request->request->get('defaultForLanguage', false), FILTER_VALIDATE_BOOL),
        ]);
        $this->em->persist($cv); $this->em->flush();
    ...
    {
        $cv->configure($request->toArray()); $this->em->flush();
    ...
    public function delete(CvDocument $cv): JsonResponse
    {
        ...
        $this->em->remove($cv); $this->em->flush();
    ...
    {
        $response = new BinaryFileResponse($this->storage->path($cv->getStoredName()));
        $response->setContentDisposition(ResponseHeaderBag::DISPOSITION_ATTACHMENT, $cv->toArray()['originalName']);
        return $response;
    }
}
