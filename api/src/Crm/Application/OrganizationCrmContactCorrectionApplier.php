...
final class OrganizationCrmContactCorrectionApplier
{
    ...
    public function apply(array $directory, iterable $corrections): array
    {
        ...
        foreach ($corrections as $correction) {
            if (!$correction instanceof CrmContactCorrection || $correction->isEmpty()) continue;
            $correctionsByContact[$this->mapKey($correction->getOrganizationKey(), $correction->getContactKey())] = $correction->toArray();
        }

        $appliedCount = 0;
        foreach ($directory['organizations'] as &$organization) {
            $organizationKey = (string) ($organization['key'] ?? '');
            if (!is_array($organization['contacts'] ?? null)) continue;
    ...
    private function mapKey(string $organizationKey, string $contactKey): string { return $organizationKey."\0".$contactKey; }
